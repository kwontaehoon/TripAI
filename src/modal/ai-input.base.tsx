...
import { useGeminiAiMutation } from "@/hooks/springboot/queries"
import { aiResponseAtom, modalUiStateAtom } from "@/store/ai"
import { transportationText } from "@/common/text"
      ...
      <div className="overflow-y-scroll scrollbar-hide">
        <div className="flex lg:justify-start items-center justify-center fixed lg:relative w-full py-3 lg:py-0 bg-white">
          ...
          <Image
            src={CloseSVG}
            alt="Close SVG"
            className="absolute left-0 lg:hidden block cursor"
        ...
        <div className="flex flex-wrap gap-5 mt-5 mb-20">
          {transportationText.map((x) => (
            <div
              key={x.id}
              className="cursor shadow-md flex flex-col items-center justify-center w-[calc((100%-40px)/3)] h-[100px] rounded-xl"
            >
        ...
        <div className="lg:flex hidden flex-1 items-end absolute bottom-0 w-full left-0 p-5 bg-white shadow">
          <div className="flex w-full">
            <div
              className="w-full h-12 w-h-center rounded-xl border mr-2 cursor"
              onClick={() =>
                setModalUiState({ ...modalUiState, aiInput: false })
              }
            >
            ...
            <div
              className="w-full h-12 w-h-center rounded-xl border ml-2 cursor"
              onClick={async () => {
